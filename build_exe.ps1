...
Write-Host "=== Style Cut Desk : build EXE ==="

Get-Process -Name "StyleCutDesk" -ErrorAction SilentlyContinue | Stop-Process -Force
Start-Sleep -Seconds 2
$old = Join-Path $PSScriptRoot "dist\StyleCutDesk.exe"
if (Test-Path $old) {
  try { Remove-Item -LiteralPath $old -Force } catch {
    throw "Cannot replace dist\StyleCutDesk.exe. Close the running app first."
  }
}
...
& $vpy -m PyInstaller --noconfirm --clean StyleCutDesk.spec

$exe = Join-Path $PSScriptRoot "dist\StyleCutDesk.exe"
if (Test-Path $exe) {
  Write-Host "OK: $exe"
